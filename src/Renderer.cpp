...
Buffer createBuffer(Renderer::VulkanState& vk, VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties)
{
    Buffer buffer;
    buffer.size = size;
...
{
    return createBuffer(vk, size, usage, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
}
    ...
    MeshMaterial material)
{
    ...
    vertices.push_back({ .position = { p0, 1.0f }, .colorAndShade = packedColor, .uvMaterial = { 0.0f, 0.0f, materialId, 0.0f } });
    vertices.push_back({ .position = { p1, 1.0f }, .colorAndShade = packedColor, .uvMaterial = { 1.0f, 0.0f, materialId, 0.0f } });
    vertices.push_back({ .position = { p2, 1.0f }, .colorAndShade = packedColor, .uvMaterial = { 1.0f, 1.0f, materialId, 0.0f } });
    vertices.push_back({ .position = { p0, 1.0f }, .colorAndShade = packedColor, .uvMaterial = { 0.0f, 0.0f, materialId, 0.0f } });
    vertices.push_back({ .position = { p2, 1.0f }, .colorAndShade = packedColor, .uvMaterial = { 1.0f, 1.0f, materialId, 0.0f } });
    vertices.push_back({ .position = { p3, 1.0f }, .colorAndShade = packedColor, .uvMaterial = { 0.0f, 1.0f, materialId, 0.0f } });
}
...
{
    appendMeshFace(vertices, { min.x, min.y, z }, { max.x, min.y, z }, { max.x, max.y, z }, { min.x, max.y, z }, color,
        1.0f, MeshMaterial::VertexColor);
}
...
void copyBuffer(Renderer::VulkanState& vk, VkBuffer src, VkBuffer dst, VkDeviceSize size, VkDeviceSize dstOffset = 0)
{
    ...
    const VkBufferCopy copyRegion { .dstOffset = dstOffset, .size = size };
...
Image createDepthImage(Renderer::VulkanState& vk)
{
    ...
    const VkMemoryAllocateInfo allocInfo {
        .sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO,
        .allocationSize = requirements.size,
        .memoryTypeIndex = findMemoryType(vk.physicalDevice, requirements.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT),
    };
...
void pickDevice(Renderer::VulkanState& vk)
{
    ...
    for (VkPhysicalDevice device : devices) {
        ...
        if (families.complete(vk.surface != VK_NULL_HANDLE) && hasSwapchain) {
...
void createDevice(Renderer::VulkanState& vk)
{
    ...
    const VkDeviceCreateInfo info {
        ...
        .enabledExtensionCount = vk.surface ? 1U : 0U,
        .ppEnabledExtensionNames = vk.surface ? extensions : nullptr,
...
void createPipeline(Renderer::VulkanState& vk)
{
    ...
    const VkShaderModule vertexShader
        = createShaderModule(vk.device, readFile(std::string(BLOCKLAB_SHADER_DIR) + "/mesh_vertex.spv"));
    const VkShaderModule fragmentShader
        = createShaderModule(vk.device, readFile(std::string(BLOCKLAB_SHADER_DIR) + "/mesh_fragment.spv"));
...
void createFramebuffers(Renderer::VulkanState& vk)
{
    if (!vk.swapchain) {
        ...
        for (OffscreenFrame& frame : vk.offscreenFrames) {
            ...
            vkCheck(vkCreateFramebuffer(vk.device, &info, nullptr, &frame.framebuffer), "vkCreateFramebuffer offscreen");
        }
        return;
    }
...
void createCommandsAndSync(Renderer::VulkanState& vk)
{
    ...
    const uint32_t commandBufferCount
        = vk.swapchain ? static_cast<uint32_t>(vk.swapchainImages.size()) : static_cast<uint32_t>(vk.offscreenFrames.size());
    ...
    if (vk.swapchain) {
        vkCheck(vkCreateSemaphore(vk.device, &semaphoreInfo, nullptr, &vk.imageAvailable), "vkCreateSemaphore image");
        vkCheck(vkCreateSemaphore(vk.device, &semaphoreInfo, nullptr, &vk.renderFinished), "vkCreateSemaphore render");
...
{
    vk.vertexCapacityBytes = 4U * 1024U * 1024U;
    vk.vertexBuffer = createDeviceBuffer(
        vk, vk.vertexCapacityBytes, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT);
    ...
    if (vk.swapchain)
        ...
        for (OffscreenFrame& frame : vk.offscreenFrames)
            frame.paramsBuffer = createHostBuffer(vk, sizeof(Renderer::RenderParams), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT);
    }
    ...
    const VkDescriptorBufferInfo vertexInfo { .buffer = vk.vertexBuffer.buffer, .offset = 0, .range = vk.vertexBuffer.size };
    const VkDescriptorBufferInfo instanceInfo { .buffer = vk.instanceBuffer.buffer, .offset = 0, .range = vk.instanceBuffer.size };
    ...
    for (uint32_t i = 0; i < paramsSetCount; ++i) {
        ...
        VkDescriptorSet paramsSet = vk.swapchain ? vk.paramsDescriptorSet : vk.offscreenFrames[i].paramsDescriptorSet;
    ...
    , m_pigMesh(createPigMesh())
{
    ...
    m_observation = {
        ...
        .handle = reinterpret_cast<uintptr_t>(m_vk->swapchain ? reinterpret_cast<void*>(m_vk->swapchain)
...
void Renderer::uploadMesh(std::span<const MeshVertex> vertices, std::size_t firstVertex)
{
    VulkanState& vk = *m_vk;
    firstVertex = std::min(firstVertex, vertices.size());
    const VkDeviceSize required = std::max<VkDeviceSize>(sizeof(MeshVertex), vertices.size() * sizeof(MeshVertex));
    if (required > vk.vertexCapacityBytes) {
        vkDeviceWaitIdle(vk.device);
        destroyBuffer(vk, vk.vertexBuffer);
        vk.vertexCapacityBytes = required * 2;
        vk.vertexBuffer = createDeviceBuffer(
            vk, vk.vertexCapacityBytes, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT);
        firstVertex = 0;
        const VkDescriptorBufferInfo vertexInfo { .buffer = vk.vertexBuffer.buffer, .offset = 0, .range = vk.vertexBuffer.size };
    ...
    const std::size_t uploadVertexCount = vertices.size() - firstVertex;
    if (uploadVertexCount == 0)
        return;
    const VkDeviceSize uploadBytes = uploadVertexCount * sizeof(MeshVertex);
    ...
    Buffer staging = createHostBuffer(vk, uploadBytes, VK_BUFFER_USAGE_TRANSFER_SRC_BIT);
    void* mapped = nullptr;
    vkCheck(vkMapMemory(vk.device, staging.memory, 0, uploadBytes, 0, &mapped), "vkMapMemory vertices");
    if (uploadVertexCount > 0)
        std::memcpy(mapped, vertices.data() + firstVertex, uploadVertexCount * sizeof(MeshVertex));
    vkUnmapMemory(vk.device, staging.memory);
    copyBuffer(vk, staging.buffer, vk.vertexBuffer.buffer, uploadBytes, firstVertex * sizeof(MeshVertex));
...
void Renderer::uploadInstances(const World& world)
{
    ...
    for (const std::unique_ptr<NPC>& character : world.characters()) {
        ...
        const float speed = std::sqrt(snapshot.velocity.x * snapshot.velocity.x + snapshot.velocity.z * snapshot.velocity.z);
        m_instances.push_back({
...
Observation Renderer::renderObservation(const World& world, const AgentState& agent)
{
    ...
    if (m_lastWorldVersion != world.version() || agentLeftMeshCache) {
        m_meshBuilder.rebuild(world, agent);
        std::vector<MeshVertex> vertices = m_meshBuilder.vertices();
        m_terrainVertexCount = static_cast<uint32_t>(vertices.size());
        m_pigVertexOffset = m_terrainVertexCount;
        m_pigVertexCount = static_cast<uint32_t>(m_pigMesh.size());
        vertices.insert(vertices.end(), m_pigMesh.begin(), m_pigMesh.end());
        uploadMesh(vertices);
